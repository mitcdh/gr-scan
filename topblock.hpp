...
class TopBlock : public gr::top_block
{
	public:
		TopBlock(double centre_freq_1, double centre_freq_2, double sample_rate, double fft_width, double bandwidth1, double bandwidth2,
			...
			lg(gr::blocks::nlog10_ff::make(10, vector_length, -20 * std::log10(vector_length) -10 * std::log10(GetWindowPower()/vector_length))),
	...
	private:
		...
		std::vector<float> GetWindow(size_t n)
		{
			...
			for (unsigned int i = 0; i < n; i++){
				w[i] = a0 - a1 * std::cos((2.0 * 3.14159 * (double)i)/(double)(n - 1)) + a2 * std::cos((4.0 * 3.14159 * (double)i)/(double)(n - 1));
			}
			return w;
		}
